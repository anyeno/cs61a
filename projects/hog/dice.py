...
def make_test_dice(*outcomes): #可以传入任意多的参数
    ...
    """
    assert len(outcomes) > 0, 'You must supply outcomes to make_test_dice'#断言输入参数个数大于0
    for o in outcomes:
        assert type(o) == int and o >= 1, 'Outcome is not a positive integer'
    index = len(outcomes) - 1  
    def dice():
        nonlocal index  #nonlocal作用就是这里的index就是上面两行的idnex
        index = (index + 1) % len(outcomes)    # 0 1 2 0 1 2 ...
