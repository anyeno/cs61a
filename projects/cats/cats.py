...
def about(topic):
    ...
    def select(paragraph):
        ...
        divided_paragraph = paragraph.split(" ")
        for i in range (len(topic)):
            for j in range (len(divided_paragraph)):
                if lower(topic[i]) == lower(divided_paragraph[j]):
                    return True
        return False
...
def accuracy(typed, reference):
    ...
    "*** YOUR CODE HERE ***"
    count_is_same = 0
    max_len = max(len(typed_words),len(reference_words))
    ...
    if len(typed_words) == 0:
        return 0.0
    return count_is_same / len(typed_words) * 100
